...
class matmul_A_B_transpose:
    @T.prim_func
    def main(a: T.handle, at: T.handle, b: T.handle, c: T.handle):
...
sch = tvm.tir.Schedule(matmul_A_B_transpose)
...
block_AT_shared = sch.cache_read(block_C, 0, "shared")
block_AT_local = sch.cache_read(block_C, 0, "local")
...
sch.compute_at(block_AT_local, k1)
sch.compute_at(block_B_local, k1)
sch.compute_at(block_AT_shared, k0)
sch.compute_at(block_B_shared, k0)

sch.reverse_compute_at(block_C_local, j1)


i, j = sch.get_loops(block_AT_shared)[-2:]
...
# BUG can't build the cuda program, cause use too many shared memory
# FIXED add a argument `at` to the arguments of prim function
rt_mod = tvm.build(sch.mod, target="cuda")
with open("__cuda__/matmul_A_B_transpose.cu", "w") as f:
